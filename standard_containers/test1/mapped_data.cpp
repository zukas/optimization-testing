...
public:
  ...
  friend bool operator==(const std::pair<int, std::shared_ptr<order>> &orderIt,
                         const QuotePredicate &pred) {
    return orderIt.second->currencyPair() == pred.m_quote.currencyPair() &&
           orderIt.second->valueDate() == pred.m_quote.valueDate() &&
           orderIt.second->inst() == pred.m_quote.inst();
  }
...
public:
  OrderPredicate(const std::shared_ptr<order> &order) : m_order(order) {}

  friend bool operator==(const std::pair<int, std::shared_ptr<order>> &orderIt,
                         const OrderPredicate &pred) {
    return orderIt.second->orderId() == pred.m_order->orderId() &&
           orderIt.second->currencyPair() == pred.m_order->currencyPair() &&
           orderIt.second->valueDate() == pred.m_order->valueDate() &&
           orderIt.second->inst() == pred.m_order->inst();
  }
...
void mapped_data::addSubscription(const maker &maker) {
  if (!validateSubscription(maker)) {
    m_subscriptions.push_back(maker);
  }
}

void mapped_data::addOrder(std::shared_ptr<order> order) {
...
const mapped_data::OrderMap mapped_data::orders() const { return m_orders; }
...
bool mapped_data::validateSubscription(const maker &maker) const {
  ...
  auto it = std::find(m_subscriptions.begin(), m_subscriptions.end(), maker);
...
bool mapped_data::validateOrder(const std::shared_ptr<order> &order) const {
